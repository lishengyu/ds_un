...
var (
	...
	gPath      = flag.String("l", "/home/udpi_log", "数安话单文件路径，各话单路径参考标准格式查找")

	//md5还原文件比对参数
	FindMd5  = flag.Bool("findmd5", false, "查找还原缺失样本")
	DemoPath = flag.String("sample", "", "还原文件全量样本，作为标准答案，查找本地是否存在未还原文件")

	//生成话单文件参数
	Generate    = flag.Bool("generate", false, "生成话单，用于数据补报")
	UseCfg      = flag.Bool("useCfg", false, "是否需要使用./conf/conf.json文件内容更新话单字段")
	BackLogPath = flag.String("back", "/home/udpi_log/ds_data_bak/logtar", "logtar话单备份文件")
	...
	//暂不使用
	PcapFile = flag.String("p", "", "需要提取的pcap文件")

	Check = flag.Bool("ck", false, "核查话单，统计相关上报信息")
...
func printFindMd5Usage() {
	...
	fmt.Fprintf(os.Stderr, "查找本地文件还原是否有缺失（通过md5比对）\n")
	fmt.Fprintf(os.Stderr, "%s -findmd5 -f /home/file_repo -sample ./sample\n", os.Args[0])
	fmt.Fprintf(os.Stderr, "输入：\n")
	fmt.Fprintf(os.Stderr, "    -f      本设备还原样本路径：/home/file_repo\n")
	fmt.Fprintf(os.Stderr, "    -sample 全量还原样本路径\n")
	fmt.Fprintf(os.Stderr, "输出:\n")
	fmt.Fprintf(os.Stderr, "    文件比对查找结果\n")
	fmt.Fprintf(os.Stderr, "    拷贝未还原的样本到指定目录下\n")
	fmt.Fprintf(os.Stderr, "++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
}

func printGenLogtar() {
	fmt.Fprintf(os.Stderr, "++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n")
	fmt.Fprintf(os.Stderr, "生成logtar文件\n")
	fmt.Fprintf(os.Stderr, "%s -generate -f ./unrecover -md5 9a6f185cea481b34de8a268068f0be4f\n", os.Args[0])
	fmt.Fprintf(os.Stderr, "输入：\n")
	fmt.Fprintf(os.Stderr, "    -f 未还原的样本文件路径\n")
	fmt.Fprintf(os.Stderr, "    -back 需要查询06cx话单的备份路径\n")
...
func main() {
	...
	if *Generate {
		...
		GenerateLogtar(*SamplePath, *BackLogPath, *Md5Record, *UseCfg)
		return
	}
